...
export class GxpEnv {
    ...
    public uploadOrGetConfigure(sftpType: SFTPType, callback: (option) => void, options: any) {
        this._sshClient = new SshClient(this.ip, this.port, this.user, this.password);
        this.sshClient.startConnection((option) => {
            const sftpParam = sftpType === SFTPType.get ? this.tmpConfigFileName : this.backupConfigFileName;
